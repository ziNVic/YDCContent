...
export async function POST(request: Request) {
  ...
  if (!apiKey) return Response.json({ error: "未读取到线上 Kimi Open Platform API Key。请联系管理员配置。" }, { status: 503 });
  ...
  if (!response.ok) {
    ...
    console.error(`Kimi Open Platform API request failed: HTTP ${response.status} ${providerMessage}`);
